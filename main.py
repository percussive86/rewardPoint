...
try:
    ...
    member_list['이정국'] = {'전화번호': '010-1111-2222', '회원번호': 111, '누적금액': 0, '포인트': 0}
    print(e)
...
def use_point(name: str):
    ...
    if name in member_list:
        if member_list[name]['포인트'] < 10000:
            messagebox.showinfo(title="에러", message=f"{name}님의 잔여포인트가 부족합니다. (1만점 단위)", icon='warning')
            return
...
def use_service(name: str, charge: int):
    ...
    if name in member_list:
        ...
        # member_list[name]['누적금액'] = member_list[name]['누적금액'] + charge
        member_list[name]['포인트'] += point

        messagebox.showinfo(title="포인트 적립", message=f"{name}님 포인트 {point}점 적립 되었습니다.")
...
# 서비스 이용 버튼 내 필요 위젯
button_svc1 = ttk.Button(frame_menu, text="남성 커트", command=lambda: use_service(name=entry_search.get(), charge=10000))
button_svc2 = ttk.Button(frame_menu, text="엄색", command=lambda: use_service(name=entry_search.get(), charge=30000))
